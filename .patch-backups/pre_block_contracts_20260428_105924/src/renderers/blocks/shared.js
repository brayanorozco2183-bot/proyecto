...
export function sanitizeToken(value, fallback = 'default') {
    if (!value)
        return fallback;
    const cleaned = String(value).trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '-');
    return cleaned || fallback;
}
export function toTelHref(phone, fallback = '#contacto') {
    if (!phone)
        return fallback;
    const digits = String(phone).replace(/[^\d+]/g, '');
    return digits ? `tel:${digits}` : fallback;
}
export function resolvePhone(context, localPhone) {
    return localPhone || context?.phone || '';
}
export function renderParagraphs(paragraphs, className = 'block__intro') {
...
export function renderSectionHeading(heading, eyebrow) {
    ...
    if (!headingHtml && !eyebrowHtml)
        return '';
    return `<header class="block__header">${eyebrowHtml}${headingHtml}</header>`;
}
export function renderCta(cta, local, input) {
...
export function wrapSectionBlock(input, innerHtml, extraClass = '') {
    const { sectionId, blockType, variant, design } = input;
    const theme = design;
    // Resolve strategy weights from theme rules
    const shell = theme.rules.shellUsage[blockType] || theme.rules.shellUsage.default;
...
export function wrapHeroBlock(input, innerHtml, extraClass = '') {
    const theme = input.design;
...
export function deriveFallbackItems(titles, body = 'Contenido listo para hidratarse desde semantic.items.') {
    return safeArray(titles)
        .filter(Boolean)
        .map((title) => ({ title, body }));
}
export function renderTable(table, className = 'block-table') {
    if (!table || !safeArray(table.columns).length || !safeArray(table.rows).length)
        return '';
    const header = table.columns.map((column) => `<th scope="col">${escapeHtml(column)}</th>`).join('');
    const rows = table.rows
        .map((row) => `<tr>${safeArray(row).map((cell) => `<td>${escapeHtml(cell)}</td>`).join('')}</tr>`)
        .join('');
    return `
