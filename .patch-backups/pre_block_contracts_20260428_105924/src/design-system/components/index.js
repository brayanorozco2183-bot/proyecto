...
function buildDefaultImage(kind, data = {}) {
...
function renderHeroImageFigure(data = {}) {
    ...
    const alt = `${data.niche || 'Servicio profesional'} en ${data.city || 'tu zona'} · apoyo visual del servicio`;
    const caption = data.caption || `Cobertura en ${data.city || 'tu zona'} y presentación visual del servicio.`;
    return `
...
export const renderHeader = ({ content, visualVariant }) => {
    ...
    const navHtml = (navItems || [
        ...
        { label: 'FAQ', href: '#faq' }
    ]).map((item) => `<a href="${item.href}" class="nav__link">${item.label}</a>`).join('');
    return `
...
export const renderHeroPremium = ({ content, heroLayout }) => {
    const { h1, subtitle, phone, city, niche, hero_eyebrow, hero_card_eyebrow, hero_card_title, hero_card_text, hero_secondary_label, trust_bullets } = content || {};
    const phoneHref = (phone || '').replace(/\s+/g, '');
    const layout = heroLayout || 'split';
    const bullets = safeArray(trust_bullets).length
        ? safeArray(trust_bullets).map((b) => `<li>${escapeHtml(b)}</li>`).join('')
        : [
    ...
    const safeCardText = escapeHtml(hero_card_text || 'Te orientamos desde el primer contacto con un enfoque práctico, local y transparente.');
    let heroBody = '';
    if (layout === 'centered') {
        heroBody = `
...
export const renderHeroCentered = ({ content }) => {
    const { h1, subtitle, city, phone, niche, hero_eyebrow, hero_card_eyebrow, hero_card_title, hero_card_text, hero_secondary_label } = content || {};
    const phoneHref = (phone || '').replace(/\s+/g, '');
    return `
...
export const renderHeroStacked = ({ content }) => {
    const { h1, subtitle, city, phone, trust_bullets, niche, hero_eyebrow, hero_card_eyebrow, hero_card_title, hero_card_text, hero_secondary_label } = content || {};
    const phoneHref = (phone || '').replace(/\s+/g, '');
    return `
    ...
    ${safeArray(trust_bullets).length
        ...
        city,
        title: h1,
        subtitle: hero_card_title || 'Cobertura local',
        slot: 'hero-default'
    })}
...
export const renderHeroCtaHeavy = ({ content }) => {
    const { h1, subtitle, phone, city, trust_bullets, niche, hero_eyebrow, hero_card_eyebrow, hero_card_title, hero_card_text, hero_secondary_label } = content || {};
    const phoneHref = (phone || '').replace(/\s+/g, '');
    return `
        ...
        <div class="el-container">
            <div class="hero__shell hero__shell--cta">
                <div class="hero__copy">
                    ...
                    <p class="hero__subtitle">${subtitle}</p>
                    <ul class="hero__bullets hero__bullets--premium">
                        ${(trust_bullets || []).map((b) => `<li>${b}</li>`).join('')}
                    </ul>
                    <div class="cta-row">
                        <a href="tel:${phoneHref}" class="cta-primary">${phone}</a>
                    </div>
...
export const renderServicesGrid = (input) => {
...
export const renderUrgencyCounter = ({ content, vibe, visualVariant }) => {
...
export const renderCtaBand = ({ content }) => {
...
export const renderFooter = ({ content, visualVariant }) => {
...
export const renderTestimonials = ({ content, vibe }) => {
    const { h2 } = content;
    const testimonials = [
...
export const renderTimeline = (input) => {
    const { content, visualVariant } = input;
...
export const renderCertificates = ({ content, vibe }) => {
...
export const renderImageFeature = ({ content, vibe, variant }) => {
