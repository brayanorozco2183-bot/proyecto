...
function buildDefaultCtaText(niche?: string): string {
...
function buildDefaultCtaNote(niche?: string, city?: string): string {
  const service = normalizeNicheLabel(niche);
  const place = city ? ` en ${city}` : '';
  return `Cuéntanos el caso, ubicación${place}, urgencia y estado actual para orientar el alcance antes de intervenir en ${service}.`;
}
...
export function adaptSectionToBlockInput({ sectionId, section, contract, data, theme }: AdaptBlockInputArgs) {
  ...
  const defaultCta = {
    text: semantic?.cta?.text || buildDefaultCtaText(niche),
    phone: semantic?.cta?.phone || data?.phone || data?.local?.phone,
    note: semantic?.cta?.note || buildDefaultCtaNote(niche, city)
  };

  const semanticItems = ensureItemsHaveBodies(semantic?.items, city, niche);
  const derivedItems = semanticItems.length ? semanticItems : deriveItemsFromH3(section, city, niche);
