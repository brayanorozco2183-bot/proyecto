...
function buildDefaultEditorialImage(kind, data = {}) {
    const title = String(data.title || data.niche || 'Servicio premium').slice(0, 36);
    const subtitle = String(data.subtitle || data.city || data.businessName || 'Cobertura local').slice(0, 40);
...
function renderEditorialImageSection(data, slot = 'editorial-default') {
    const src = buildDefaultEditorialImage('section', {
        niche: data.niche,
        city: data.city,
        businessName: data.businessName,
        title: data.niche || 'Servicio premium',
        subtitle: data.city || data.businessName || 'Cobertura local'
    });
    const alt = `${data.niche || 'Servicio profesional'} en ${data.city || 'tu zona'} · imagen de apoyo`;
    return `
    <section class="editorial-visual-strip" data-image-slot="${slot}">
        <div class="el-container">
            <figure class="editorial-visual-card">
                <div class="editorial-visual-card__frame">
                    <img src="${src}" alt="${alt}" loading="lazy" decoding="async">
                </div>
                <figcaption class="editorial-visual-card__caption">Apoyo visual del servicio y del área de cobertura.</figcaption>
            </figure>
        </div>
    </section>`;
}
...
function generateThemeCssVariables(theme) {
    ...
    const sectionY = rhythmSpacing?.sectionY
        || (verticalRhythm === 'spacious' || verticalRhythm === 'airy'
            ? 'clamp(7.5rem, 11vw, 9.5rem)'
            : verticalRhythm === 'compact'
                ? 'clamp(4.75rem, 7vw, 6rem)'
                : 'clamp(6rem, 9vw, 8rem)');
    const blockGap = rhythmSpacing?.cardGap
        || (verticalRhythm === 'spacious' || verticalRhythm === 'airy' ? '2.25rem' : verticalRhythm === 'compact' ? '1.25rem' : '1.75rem');
    const contentGap = rhythmSpacing?.contentGap
        || (verticalRhythm === 'spacious' || verticalRhythm === 'airy' ? '2.75rem' : verticalRhythm === 'compact' ? '1.5rem' : '2.1rem');
    const shellPadding = rhythmSpacing?.shellPadding
        || (verticalRhythm === 'spacious' || verticalRhythm === 'airy'
...
function renderBreadcrumbs(items) {
    ...
    return `
                ...
                <ol class="el-breadcrumbs__list" itemscope itemtype="https://schema.org/BreadcrumbList">
                    ${items.map((item, idx) => `
                        <li class="el-breadcrumbs__item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                            <a class="el-breadcrumbs__link" href="${item.url}" itemprop="item"${idx === items.length - 1 ? ' aria-current="page"' : ''}>
        ...
        <style>
            ...
            .el-breadcrumbs__item + .el-breadcrumbs__item::before { content: '/'; color: #94a3b8; }
            ...
            .el-breadcrumbs__link[aria-current="page"] { color: var(--text); pointer-events: none; }
        </style>
    `;
}
export function renderPage(seedData, data, sections, family, extraScripts = '', pageType = 'service', heroVariant, pageComposition, seo, breadcrumbs = [], layoutContract, resolvedPlan) {
    if (!resolvedPlan) {
    ...
    const title = data.title || data.h1 || 'Servicio Local';
    const phone = data.phone || data.local_nap?.phone || '';
    ...
    const dna = {
        ...
        visualSystem: resolvedPlan.visualSystem.visualSystem
    ...
    const globalStyles = `
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .site-header__inner { 
            display: flex; justify-content: space-between; align-items: center; gap: 1rem; padding: 0.6rem 1rem; /* Point 4 */
        ...
        .nav__cta { 
            height: 2.85rem !important; /* Point 4 - Slightly more compact header CTA */
        ...
        .trust-strip__label { font-size: 0.75rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.12em; color: var(--muted); opacity: 0.7; }
        ...
        .map-block__frame--panorama,
        .map-block--full .map-block__frame,
        .section--map .map-wrapper {
            min-height: clamp(340px, 44vw, 540px);
        }
        .map-block__frame--boxed,
        .map-grid .map-boxed-wrapper {
            min-height: clamp(320px, 40vw, 500px);
        }
        .map-block__frame--spotlight {
            min-height: clamp(340px, 42vw, 540px);
        }
        .map-block__frame--minimal,
        .map-block--minimal .map-block__frame {
        ...
        .rich-content ul { list-style: none; margin-bottom: 2rem; }
        .rich-content li { position: relative; padding-left: 2rem; margin-bottom: 1rem; font-weight: 500; }
        .rich-content li::before { content: "✓"; position: absolute; left: 0; color: var(--primary); font-weight: 900; font-size: 1.2rem; top: -2px; }
        ...
        .footer__group-title { font-size: 0.75rem; font-weight: 900; text-transform: uppercase; letter-spacing: 0.2em; color: #64748b; margin-bottom: 2rem; }
        ...
        .footer__subtitle { color: rgba(255, 255, 255, 0.56); font-weight: 800; margin-bottom: 1rem; text-transform: uppercase; font-size: 0.78rem; letter-spacing: 0.12em; }
        ...
        @media (max-width: 1024px) {
            ...
            .semantic-items, .services-grid, .trust-grid, .services-grid__cards, .local-proof__grid, .process-steps__timeline, .footer__grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        }
        @media (max-width: 768px) {
            ...
            .site-header__inner { justify-content: center; border-radius: 1.2rem; padding: 1rem; }
            .nav { justify-content: center; }
            .hero--split .hero__shell { grid-template-columns: 1fr !important; gap: 3rem; text-align: center; direction: ltr !important; }
            ...
            .footer__grid, .semantic-items, .services-grid, .trust-grid, .services-grid__cards, .local-proof__grid, .process-steps__timeline { grid-template-columns: 1fr; text-align: left; gap: 1rem; }
    ...
    const navItems = sections
        .filter(s => ['services_grid', 'urgency_panel', 'local_proof', 'faq'].includes(s.blockType))
        .map(s => {
        const labelMap = {
        ...
        return {
            label: labelMap[s.blockType] || 'Detalles',
            href: `#${s.sectionId || slugify(s.h2 || 'section')}`
        };
    })
        .slice(0, 4);
    const bodyParts = [];
    // 1. Header (Varía según skeleton)
    const headerVariant = skeleton === 'conversion-funnel' ? 'glass' : (dna.navbarTreatment || 'glass');
    bodyParts.push(components.renderHeader({
        tokens: tokens,
        content: { businessName: data.businessName, phone, navItems },
    ...
    // 2. Hero
    const heroData = data.hero || { h1: title, subtitle: data.subtitle, phone, city };
    const heroRenderData = {
        ...
        heroLayout: rules?.hero?.template || macroHeroTemplate || heroVariant,
        visualVariant: rules.hero.template
    };
    const heroLayoutStr = (rules?.hero?.template || macroHeroTemplate || heroVariant || 'split');
    ...
    if (skeleton !== 'premium-showcase' && macroProof !== 'distributed') {
        bodyParts.push(components.renderTrustStrip({ tokens: tokens, content: {} }));
    }
    ...
    sections.forEach((s, idx) => {
        const salt = (s.sectionId || s.h2 || '').split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
        const sectionId = s.sectionId || slugify(s.h2 || `section-${salt}`);
        if (s.blockType === 'hero_trust')
            return;
        // Priorizar el contrato del ResolvedPlan
        const contract = resolvedPlan?.sections.find((sc) => sc.sectionId === sectionId) || layoutContract?.sections?.[sectionId] || {};
        ...
        const blockType = String(s.blockType || '');
        ...
        const isRegistryBlock = s.blockType && (s.blockType.toString() in blockRegistry);
        if (isRegistryBlock) {
            const renderer = blockRegistry[s.blockType];
            ...
            try {
                ...
                console.error(`[ProceduralEngine] Error rendering block ${s.blockType}:`, err);
                // Fallback to legacy
                bodyParts.push(`<div class="el-section-wrapper ${toneClass}">${s.html}</div>`);
            }
        }
        else if (s.html.includes('section-shell')) {
            ...
            if (s.blockType === 'map') {
                bodyParts.push(components.renderMap({
                    tokens: tokens,
                    content: { h2: s.h2, city: data.city, address: data.local?.address, description: s.html },
                    vibe: 'minimal',
                    id: sectionId
                }));
            }
    ...
    if (skeleton !== 'editorial-longform') {
        ...
        bodyParts.push(components.renderUrgencyCounter({
            tokens: tokens,
            content: { h2: '¿Necesitas un especialista ahora?', phone },
            visualVariant: finalUrgencyVariant
        }));
    }
    // 6. Footer
    const footerVariant = skeleton === 'local-directory' ? 'directory' : dna.footerTreatment;
    bodyParts.push(components.renderFooter({
        tokens: tokens,
        content: { businessName: data.businessName, city, phone },
...
<head>
    ...
    <meta name="description" content="${seo?.metaDescription || data.subtitle || ''}">
    ...
    <meta property="og:description" content="${seo?.openGraph?.description || seo?.metaDescription || data.subtitle || ''}">
    <meta property="og:url" content="${seo?.openGraph?.url || seo?.canonical || ''}">
    <meta property="og:site_name" content="${seo?.openGraph?.siteName || data.businessName || ''}">
    ...
    <meta name="twitter:description" content="${seo?.twitter?.description || seo?.metaDescription || data.subtitle || ''}">
    ...
    ${(() => {
        ...
        const extract = (f) => f.split(',')[0].replace(/['"]/g, '').trim();
        if (tokens.typography.fontDisplay)
            families.add(extract(tokens.typography.fontDisplay));
        if (tokens.typography.fontBody)
            families.add(extract(tokens.typography.fontBody));
    ...
    <style>
        ...
        .pattern-section_quote_proof blockquote::after { content: "CERTIFICADO"; position: absolute; top: -1rem; right: 0; background: var(--primary); color: white; padding: 0.5rem 1rem; font-size: 0.7rem; font-style: normal; font-weight: 800; }
...
<body class="page-visual-mode--premium ${surfaceClass} ${systemClass} ${macroClasses}">
    ...
    fullHtml = fullHtml
        ...
        .replace(/\{\{NAP\}\}/gi, data.businessName || '')
        .replace(/\{\{BUSINESS_NAME\}\}/gi, data.businessName || '')
        .replace(/\[\[CITY\]\]/gi, city);
