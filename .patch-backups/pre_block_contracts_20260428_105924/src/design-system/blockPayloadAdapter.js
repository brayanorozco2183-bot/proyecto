...
function sanitizeTrustLabel(value, city) {
...
function deriveItemsFromH3(section) {
    const h3s = asArray(section?.h3s);
    return h3s
        .filter(Boolean)
        .slice(0, 6)
        .map((title) => ({
...
function deriveFaqsFromH3(section) {
    const h3s = asArray(section?.h3s);
    return h3s
        .filter(Boolean)
        .slice(0, 6)
...
function deriveTrustBullets(section, data) {
    ...
    const direct = asArray(semantic?.trustBullets || semantic?.bullets).filter(Boolean);
    const globalPool = asArray(data?.intentModel?.mandatoryTrustElements || data?.trustAssets).filter(Boolean);
...
function deriveMapNote(section, data) {
    const semantic = section?.metadata?.semantic || {};
    if (semantic?.mapNote)
        return semantic.mapNote;
...
export function adaptSectionToBlockInput({ sectionId, section, contract, data, theme }) {
    const semantic = section?.metadata?.semantic || {};
