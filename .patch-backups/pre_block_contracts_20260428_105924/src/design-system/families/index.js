...
export const DEFAULT_FAMILY_ID = 'minimal_authority';
...
export const FAMILY_LIBRARY = Object.freeze(registry);
const cssOverrides = {
    editorial: css `
        :root {
...
export const familyCSSOverrides = Object.freeze(cssOverrides);
function normalizeFamilyId(input) {
    if (!input)
        return null;
    const normalized = input.trim().toLowerCase().replace(/-/g, '_');
    return (normalized in FAMILY_LIBRARY ? normalized : null);
}
export function hasFamilyDefinition(familyId) {
    return normalizeFamilyId(familyId) !== null;
}
export function getFamilyDefinition(familyId) {
    const normalized = normalizeFamilyId(familyId);
    return FAMILY_LIBRARY[normalized ?? DEFAULT_FAMILY_ID];
}
export function getFamilyCSSOverride(familyId) {
    const normalized = normalizeFamilyId(familyId);
    return familyCSSOverrides[normalized ?? DEFAULT_FAMILY_ID];
}
export function getFamilyIds() {
    return FAMILY_IDS;
}
function validateFamilyRegistry() {
    for (const id of FAMILY_IDS) {
        const definition = FAMILY_LIBRARY[id];
        if (definition.id !== id) {
            throw new Error(`[design-system/families] Family id mismatch: registry key "${id}" !== definition.id "${definition.id}".`);
        }
        if (!(id in familyCSSOverrides)) {
            throw new Error(`[design-system/families] Missing CSS override entry for family "${id}".`);
        }
    }
}
