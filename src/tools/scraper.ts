...
export class SERPManager {
    ...
    async scrapeUnifiedSERP(query: string, city: string): Promise<{ organic: LocalSearchResult[], localPack: LocalBusinessData[] }> {
        ...
        // 1. Intentar vía SDI (PowerShell)
        const sdiResult = await new Promise<{ organic: LocalSearchResult[], localPack: LocalBusinessData[] }>((resolve) => {
            ...
            exec(psCommand, async (error: any, stdout: string, stderr: string) => {
                if (fs.existsSync(outputFile)) {
                    ...
                    const links = this.extractLinksFromRawSource(content, fullQuery, city);
                    const organic = links.map((link, i) => ({
        ...
        if (sdiResult.organic.length >= 5) {
            console.log(`[STEALTH] SDI Exitoso. ${sdiResult.organic.length} enlaces capturados.`);
            return sdiResult;
        }

        // 2. Fallback: Playwright Direct Stealth (Si SDI falla o devuelve poco)
        console.warn(`[STEALTH] SDI insuficiente (${sdiResult.organic.length}/5). Iniciando Fallback Direct Stealth...`);
        
        await ensureScraperDeps();
        const { browser, context } = await this.createIncognitoContext();
        const page = await context.newPage();
        const cursor = await ghostCursor(page);

        try {
            await this.warmupSession(page, cursor);
            const urlGoogle = `https://www.google.com/search?q=${encodeURIComponent(fullQuery)}`;
            await page.goto(urlGoogle, { waitUntil: 'domcontentloaded', timeout: 30000 });
            
            await this.handleGoogleConsent(page, cursor);
            await this.delay(2000, 4000);

            const content = await page.content();
            const links = this.extractLinksFromRawSource(content, fullQuery, city);
            
            const organic: LocalSearchResult[] = links.map((link, i) => ({
                title: `Resultado Stealth ${i + 1}`,
                link: link,
                snippet: 'Capturado vía Playwright Stealth Fallback',
                position: i + 1
            }));

            console.log(`[STEALTH] Fallback Finalizado. ${organic.length} enlaces capturados.`);
            return { organic, localPack: [] };
        } catch (err: any) {
            console.error(`[STEALTH] Fallback falló: ${err.message}`);
            return sdiResult; // Devolver lo que tengamos
        } finally {
            await browser.close().catch(() => {});
        }
    }

    private extractLinksFromRawSource(content: string, query: string, city: string): string[] {
        ...
        const missionSafeLinks = filterMissionRelevantUrls(links, query, city);
        return (missionSafeLinks.length ? missionSafeLinks : links).slice(0, 10);
    }
