import { loadNichePlaybook } from './playbookLoader.js';
...
export function getPremiumNicheContract(niche: string): PremiumNicheContract {
  ...
  const playbook = loadNichePlaybook(niche || '') || buildGenericNichePlaybook(niche || 'servicio local');
  const profile = buildNicheIntelligenceProfile({ niche: niche || playbook.displayName });
  return {
    niche: playbook.id,
    scoreFloor: profile.legalSensitivity === 'high' ? 88 : 85,
    allowedTerms: unique([playbook.displayName, ...playbook.aliases, ...playbook.allowedVocabulary, ...playbook.coreServices, ...profile.technicalVocabulary]),
    forbiddenTerms: unique([...playbook.forbiddenVocabulary, ...profile.forbiddenClaims]),
    requiredSections: ['servicios', 'proceso', 'precios orientativos', 'cobertura local', 'faq'],
    requiredFaqSignals: unique([...playbook.coreServices.slice(0, 4), ...profile.faqSeeds.slice(0, 3)]),
    premiumAngles: unique([...(playbook.toneProfile?.editorialAngles || []), ...playbook.decisionCriteria.map(c => c.whyItMatters), ...playbook.validTrustSignals.map(s => s.label), ...profile.decisionCriteria, ...profile.trustAssets, ...profile.safeClaimAlternatives]).slice(0, 16)
  };
}
