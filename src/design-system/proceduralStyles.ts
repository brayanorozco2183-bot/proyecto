...
import { BLOCK_VISUAL_RECOVERY_CSS } from './blockVisualRecoveryCss.js';
import { DETERMINISTIC_PREMIUM_CSS } from './deterministicPremiumCss.js';
import { DELIVERY_STABLE_CSS } from './deliveryStableCss.js';
...
export function getProceduralGlobalStyles(): string {
    ...
    let styles = cachedProceduralGlobalStyles || '';

    if (!styles.includes('Gravity Conversion UX Super Patch')) {
        styles = `${styles}\n${PROCEDURAL_CONVERSION_UX_PATCH}`;
    }

    if (!styles.includes('Gravity Block Visual Recovery v1')) {
        styles = `${styles}\n${BLOCK_VISUAL_RECOVERY_CSS}`;
    }

    if (!styles.includes('Gravity Deterministic AI Kernel')) {
        styles = `${styles}\n${DETERMINISTIC_PREMIUM_CSS}`;
    }

    if (!styles.includes('Gravity Delivery Stable CSS')) {
        styles = `${styles}\n${DELIVERY_STABLE_CSS}`;
    }
