...
function buildDefaultEditorialImage(kind: 'hero' | 'section', data: {
    niche?: string;
    city?: string;
    businessName?: string;
    title?: string;
...
function renderEditorialImageSection(data: { niche?: string; city?: string; businessName?: string }, slot = 'editorial-default'): string {
    ...
    <section class="editorial-visual-strip" data-image-slot="${slot}">
        <div class="el-container">
            <figure class="editorial-visual-card">
                ...
                <figcaption class="editorial-visual-card__caption">Apoyo visual del servicio y del área de cobertura. Sustituible por imagen real cuando exista material propio.</figcaption>
            </figure>
        </div>
    </section>`;
}
        ...
        <nav class="el-breadcrumbs" aria-label="Breadcrumb">
            <div class="el-container">
                <ol class="el-breadcrumbs__list" itemscope itemtype="https://schema.org/BreadcrumbList">
                    ...
                    `).join('<span class="el-breadcrumbs__separator" aria-hidden="true">/</span>')}
                </ol>
            </div>
        </nav>
        <style>
            ...
            .el-breadcrumbs__separator { color: #94a3b8; }
            .el-breadcrumbs__item:last-child .el-breadcrumbs__link { color: var(--text); pointer-events: none; }
            .el-breadcrumbs__item:last-child + .el-breadcrumbs__separator { display: none; }
        </style>
    `;
}
...
export function renderPage(
    ...
    const navItems = sections
        .filter(s => ['services_grid', 'urgency_panel', 'local_proof', 'faq'].includes(s.blockType as any))
        .map(s => {
            ...
            return {
                label: labelMap[s.blockType as string] || 'Detalles',
    ...
    const heroRenderData = {
        ...
        heroLayout: (rules?.hero?.template as any) || macroHeroTemplate || (heroVariant as any),
        visualVariant: rules.hero.template
    };

    const heroLayoutStr = ((rules?.hero?.template as any) || macroHeroTemplate || (heroVariant as any) || 'split');
    ...
    sections.forEach((s, idx) => {
        const sectionId = s.sectionId || slugify(s.h2);
        if (s.blockType === 'hero_trust') return;
        ...
        if (isRegistryBlock) {
            const renderer = blockRegistry[s.blockType as keyof typeof blockRegistry];
