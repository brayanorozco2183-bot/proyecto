...
export interface PremiumScoreInput { html: string; city: string; niche: string; phone?: string; businessName?: string; qualityGate?: any; }
...
export function scorePremiumOutput(input: PremiumScoreInput): PremiumScoreResult {
  ...
  if ($('a[href^="tel:"], button, .cta, [data-cta]').length < 2) { issueCodes.push('CTA_WEAK'); breakdown.conversion -= 7; }
  if (!/viewport/i.test(String($('meta[name="viewport"]').attr('content') || ''))) { issueCodes.push('MOBILE_VIEWPORT_MISSING'); breakdown.uxMobile -= 6; }
