...
function severityFromTechnicalCode(code: string): QualitySeverity {
  ...
  if (/PREMIUM_.*(?:EMPTY|MISSING|INVALID|PLACEHOLDER|SYSTEM_LEAK|FAKE_TEL|UNVERIFIED_REVIEW)/.test(upper)) return 'major';
  if (/PREMIUM_/.test(upper)) return 'major';
...
function suggestedActionForTechnical(code: string): string {
  const upper = String(code || '').toUpperCase();
  if (/PREMIUM_/.test(upper)) return 'Corregir la checklist premium antes de publicar esta página.';
...
function fromTechnical(state: PipelineState, phase: CanonicalPipelinePhaseId): QualityIssue[] {
  return extractValidationErrors(state)
    ...
    .map((entry) => {
      const bracketed = parseBracketedMessage(entry);
      const parts = entry.split(':');
      const code = bracketed ? bracketed.code : String(parts[0] || 'TECHNICAL_VALIDATION').trim();
      const message = bracketed ? bracketed.message : (parts.length > 1 ? entry : code);
