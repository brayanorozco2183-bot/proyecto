...
function removeDuplicateTerminalUrgency($: cheerio.CheerioAPI, issues: PageCoherenceIssue[]): void {
  ...
  const candidates = $('section.el-section.vibe-premium, section.el-section.vibe-local, section.el-section.vibe-minimal').filter((_i, el) => {
    const $el = $(el);
    return $el.find('.urgency-banner').length > 0 && $el.closest('[data-block-type]').length === 0;
  });
  candidates.each((_i, el) => $(el).remove());
...
function dedupePillsAndCtas($: cheerio.CheerioAPI, issues: PageCoherenceIssue[]): void {
  ...
  let removedPills = 0;
  $('.block__pill, .trust-pill-refined').each((_i, el) => {
  ...
  $('a.cta-primary, [data-cta="primary"]').each((_i, el) => {
    const $el = $(el);
...
function extractVisibleFaq($: cheerio.CheerioAPI): Array<{ question: string; answer: string }> {
  ...
  faqRoot.find('details').each((_i, el) => {
    const $el = $(el);
  ...
  faqRoot.find('.faq-entry').each((_i, el) => {
    const $el = $(el);
...
function cleanFaqNode(node: any, visibleFaq: Array<{ question: string; answer: string }>): boolean {
  ...
  const source = visibleFaq.length ? visibleFaq.map((item) => ({
    '@type': 'Question',
    name: item.question,
