...
function getVisibleTestimonialCards($: cheerio.CheerioAPI): Array<{ name: string; text: string; meta: string; sectionMode: string; source: string }> {
  return $('.testimonial-card')
    ...
    .get()
    .filter((item) => item.name || item.text || item.meta);
}
...
function detectPerformanceIssues($: cheerio.CheerioAPI, html: string, styles: string): string[] {
  ...
  const missingDimensions = $('img').filter((_i, el) => !$(el).attr('width') || !$(el).attr('height')).length;
  const eagerNonHero = $('img[loading="eager"]').filter((_i, el) => $(el).closest('.hero-immersive__media, .hero-visual__frame').length === 0).length;
  const nonLazyIframes = $('iframe').filter((_i, el) => String($(el).attr('loading') || '').toLowerCase() !== 'lazy').length;
...
export function validateRenderedPageTechnically(renderedPage: { html: string }, context: TechnicalSeoValidationInput): string[] {
  ...
  if ($('.el-breadcrumbs [itemprop="name"]').filter((_i, el) => !normalizeSpace($(el).text())).length > 0) {
  ...
  if (visibleQuestions.length > 0 && schemaQuestions.length > 0) {
    ...
    const missingInSchema = visibleQuestions.filter((q) => !schemaSet.has(normalizeText(q)));
    const missingInVisible = schemaQuestions.filter((q) => !visibleSet.has(normalizeText(q)));
  ...
  const bodyAndAttrs = `${$('body').text()} ${$('[alt], [title], [aria-label], [placeholder]').map((_i, el) => $(el).attr('alt') || $(el).attr('title') || $(el).attr('aria-label') || $(el).attr('placeholder') || '').get().join(' ')}`;
  ...
  if (mapIframes.length > 0) {
    const srcs = mapIframes.map((_i, el) => String($(el).attr('src') || '')).get().join(' ');
  ...
  if (context.expectedPhone) {
    ...
    const telLinks = $('a[href^="tel:"]').map((_i, el) => ($(el).attr('href') || '').replace(/\D+/g, '')).get();
    const phoneFound = normalizedPhone && (bodyDigits.includes(normalizedPhone) || telLinks.some((value) => value.includes(normalizedPhone)));
