...
export class IntegrityGuard {
    ...
    static check(draft: ContentDraft, context: IntegrityContext = {}): string[] {
        ...
        for (const block of normalizedDraft.blocks) {
            ...
            if (/<(?:html|head|body)\b/i.test(html)) {
                issues.push(`INTEGRITY:DOCUMENT_WRAPPER:block_${sectionIndex}`);
            }
    ...
    static isHtmlSafe(html: string): boolean {
        ...
        try {
            ...
            const wrapperMatch = html.match(/<(?:html|head|body)\b/i);
            if (wrapperMatch) {
                console.warn(`[IntegrityGuard] Rejected: html/head/body wrapper found. Match: "${wrapperMatch[0]}" at index ${wrapperMatch.index}`);
                return false;
            }
    ...
    private static sanitizeBlock(block: ContentBlock, index: number, context: IntegrityContext): ContentBlock {
        ...
        const semantic = metadata.semantic !== undefined
            ? this.sanitizeSemanticValue(metadata.semantic, context)
            : metadata.semantic;
    ...
    private static unwrapDocumentHtml(html: string): string {
        ...
        return String(bodyHtml || $.root().html() || '').trim();
    }
    ...
    private static sanitizeSemanticValue(value: any, context: IntegrityContext): any {
        if (Array.isArray(value)) {
            return value.map((item) => this.sanitizeSemanticValue(item, context));
        }
