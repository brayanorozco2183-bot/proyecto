...
function syncFooterBrandWithSeo($: cheerio.CheerioAPI, city: string, niche?: string): void {
  ...
  $('.footer__brand-name, .site-header__brand-name, .brand__name').each((_i, el) => {
...
function ensureInternalLinkAnchors($: cheerio.CheerioAPI): void {
  $('.internal-links-item').each((_i, el) => {
    const $el = $(el);
...
function shortenOverlyLongButtons($: cheerio.CheerioAPI): void {
  ...
  $('a.cta-primary, .internal-links-item__anchor, .block__cta-group a, a.section-cta').each((_i, el) => {
    const $el = $(el);
...
function dedupeRepeatedTrustSignals($: cheerio.CheerioAPI): void {
  ...
  $('.block__pill, .hero-block__trust .pill, .trust-band__item, .hero-proof-card__eyebrow').each((_i, el) => {
    const node = $(el);
...
function sanitizeFaqVisibleAndSchema($: cheerio.CheerioAPI): void {
  $('.faq-item, .faq-card, .faq-entry, [data-faq-item], details').each((_i, el) => {
    const $el = $(el);
