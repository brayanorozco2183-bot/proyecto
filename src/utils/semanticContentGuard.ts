...
export function repairBlockHtmlSemantics(html: string, contract: SectionQualityContract & { profile?: NicheIntelligenceProfile }): string {
  ...
  (root.find('*') as any).contents().each((_: number, node: any) => {
    ...
    // Fix "Comunidad de" if it's orphaned or at the end of a phrase
    text = text.replace(/\bComunidad de\s*$/i, `Comunidad de ${contract.city}`);
    text = text.replace(/\bComunidad de\s+([,.;:!?¡¿])/gi, `Comunidad de ${contract.city}$1`);
    text = text.replace(/\bComunidad de\s+<\/li>/gi, `Comunidad de ${contract.city}</li>`);
    
    node.data = sanitizeLegalRiskClaims(text, contract.profile || contract.niche).html;
  });
...
export function guardContentBlock(args: {
  block: ContentBlock;
  contract: SectionQualityContract & { profile?: NicheIntelligenceProfile };
...
export function enforceDraftSemanticCoherence(draft: ContentDraft, context: NormalizedContext, plan?: PagePlan, profile?: NicheIntelligenceProfile): ContentDraft {
  ...
  const blocks = draft.blocks.map((block, index) => {
    ...
    const contract = buildSectionQualityContract({ section, sectionId: block.id, index, totalSections: draft.blocks.length, context, profile });
...
export function repairRenderedSemanticQuality(html: string, args: { city: string; niche: string; canonical?: string; businessName?: string; phone?: string; profile?: NicheIntelligenceProfile }): string {
  const $ = cheerio.load(html || '', { decodeEntities: false } as any);
  ...
  ($('body *') as any).contents().each((_: number, node: any) => {
    ...
    // Final rendered repair for "Comunidad de"
    text = text.replace(/\bComunidad de\s*$/i, `Comunidad de ${args.city}`);
    text = text.replace(/\bComunidad de\s+([,.;:!?¡¿])/gi, `Comunidad de ${args.city}$1`);
    
    node.data = sanitizeLegalRiskClaims(text, args.profile || args.niche).html;
  });
...
export function finalizeRenderedPageSemantics(page: RenderedPage, args: { mission: GenerationMission; pagePlan?: PagePlan; context?: NormalizedContext; canonical?: string; profile?: NicheIntelligenceProfile }): RenderedPage {
  const city = args.context?.city || args.mission.city;
  const niche = args.context?.niche || args.mission.niche;
  const profile = args.profile || (args.mission as any).nicheProfile;
  const canonical = args.canonical || (page.metadata?.seo as any)?.canonical || (args.pagePlan?.seoBrief as any)?.canonicalSlug;
  const legalSafeHtml = sanitizeLegalRiskClaims(page.html, profile || niche).html;
