...
export class ContentGenerationPipeline {
    ...
    public async runEnrichmentPhase(draft: ContentDraft, context: NormalizedContext, plan: PagePlan, mission: GenerationMission): Promise<ContentDraft> {
        return this.writingEngine.runEnrichment(draft, context, plan, mission);
    }
    ...
    private createStateMachineHost(): PipelineHostAdapter {
        return {
            ...
            runEnrichmentPhase: (d, c, p, m) => this.writingEngine.runEnrichment(d, c, p, m),
            ...
            updatePhaseStatus: async (m, phase) => {
                const db = await dbManager.getDB();
                await db.run('UPDATE city_data SET status = ? WHERE mission_id = ? AND city = ?', [phase.toUpperCase(), m.missionId, m.city]);
            }
        };
    }
    ...
    private sanitizeMission(mission: GenerationMission): GenerationMission {
        ...
        return attachMissionRuntimeContext(mission);
    }
    ...
    private async applyPlaybookContext(mission: GenerationMission) {
        ...
        const profile = buildNicheIntelligenceProfile({
            ...
            localAreas: mission.cluster_data?.geo?.map(g => g.name)
        });
        ...
        // Inject profile into relevant agents if they support it
        if ((this.architect as any).setNicheProfile) (this.architect as any).setNicheProfile(profile);
        if ((this.writer as any).setNicheProfile) (this.writer as any).setNicheProfile(profile);
        if ((this.qualityScoreAgent as any).setNicheProfile) (this.qualityScoreAgent as any).setNicheProfile(profile);
        if ((this.spanishCorrector as any).setNicheProfile) (this.spanishCorrector as any).setNicheProfile(profile);
        if ((this.nicheCoherenceAgent as any).setNicheProfile) (this.nicheCoherenceAgent as any).setNicheProfile(profile);

        (mission as any).nicheProfile = profile;

        return { mission, playbookContext: { ...playbook, profile } };
    }
    ...
    private extractFaqsFromRenderedHtml(html: string) {
        ...
        const faqs: any[] = [];
        const faqRoot = $('[data-block-type="faq"], section#faq, .block--faq').first();
        const scope = faqRoot.length ? faqRoot : $('body');
        scope.find('.faq-item, .faq-entry, details.faq-item').each((_: any, el: any) => {
            const q = $(el).find('h3, summary .faq-question-text, summary').first().text().trim();
            const a = $(el).find('.faq-answer, .faq-content-refined, .faq-content, p').last().text().trim();
            if (q && /[¿?]/.test(q) && q.length >= 12 && !/^(menú|menu|inicio|servicios|áreas|areas|dudas|contacto)$/i.test(q) && a) faqs.push({ q, a });
        });
    ...
    private generateSoftModeResult(page: RenderedPage, mission: GenerationMission, error: Error, obs: ObservabilityMetadata): PipelineResult {
        return { success: true, data: { html: page.html, html_path: '', word_count: this.countWordsFromRenderedHtml(page.html), score: 0, notes: [`ERROR: ${error.message}`], status: 'error_soft' as any, url: '', h1: this.extractPrimaryH1FromHtml(page.html), faqs: [], metadata: { ...page.metadata, is_soft: true, h1: this.extractPrimaryH1FromHtml(page.html), total_words: this.countWordsFromRenderedHtml(page.html), issues: page.metadata.validation_errors || [], observability: obs }, observability: obs } };
    }
